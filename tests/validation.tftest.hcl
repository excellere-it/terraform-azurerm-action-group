...
#
# Test: Valid Expiration Days
#
# Verifies that a valid expiration_days value (> 0) is accepted.
#
run "test_valid_expiration_days" {
  ...
  assert {
    ...
    error_message = "Valid expiration days should be accepted"
  }
}

#
# Test: Default Expiration Days
#
# Verifies that the default expiration_days value (365) is used when not specified.
#
run "test_default_expiration_days" {
  command = plan
  ...
  assert {
    ...
    error_message = "Default expiration days should be 365"
  }
}

#
# Test: Zero Expiration Days
#
# Verifies that expiration_days = 0 is rejected.
# This should fail the validation rule.
#
run "test_zero_expiration_days" {
  command = plan

  variables {
    display_name    = "Test"
    expiration_days = 0 # Should fail validation
...
#
# Verifies that negative expiration_days values are rejected.
# This should fail the validation rule.
#
run "test_negative_expiration_days" {
  command = plan

  variables {
    display_name    = "Test"
    expiration_days = -10 # Should fail validation
