# Contributing to terraform-azurerm-action-group

Thank you for your interest in contributing to the terraform-azurerm-action-group module! This document provides guidelines and instructions for contributing.
...
2. Clone your fork locally:
   ```bash
   git clone https://github.com/YOUR_USERNAME/terraform-azurerm-action-group.git
   cd terraform-azurerm-action-group
   ```
3. Add the upstream repository:
   ```bash
   git remote add upstream https://github.com/kelomai/terraform-azurerm-action-group.git
   ```
...
Example:
```
feat: Add email receiver support to action group

Added email_receiver block configuration to enable email notifications
...
Tests use Terraform's native test framework (`.tftest.hcl` files):

```hcl
run "test_your_feature" {
  command = plan

  assert {
    condition     = output.id != ""
    error_message = "Action group ID should not be empty"
  }
}
```
...
1. **Variables**: All variables must have descriptions
   ...
   variable "display_name" {
     type        = string
     description = "The display name of the action group (max 12 characters)"
   }
   ```
...
3. **Comments**: Use comments for complex logic
   ```hcl
   # Generate standardized resource name using the namer module
   # Format: ag-{environment}-{location}-{workload}-{instance}
   ```
...
variable "display_name" {
  ...
  validation {
    ...
    error_message = "Display name must be between 1 and 12 characters"
  }
}
```
...
- Check existing [issues](https://github.com/kelomai/terraform-azurerm-action-group/issues)
...
Thank you for contributing to terraform-azurerm-action-group!
